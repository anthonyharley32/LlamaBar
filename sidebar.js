...
const currentLogLevel =
  parseInt(localStorage.getItem("logLevel")) || LogLevel.INFO;
...
const logTrace = (category, message, data) =>
...
try {
  ...
  const explainButtonContainer = document.getElementById(
  ...
  function isLocalModel(modelName) {
    ...
    return localPrefixes.some((prefix) =>
      modelNameWithoutPrefix.toLowerCase().startsWith(prefix.toLowerCase())
    );
  }
  ...
  function showSetupWizard() {
    ...
    setupWizardFrame.src = chrome.runtime.getURL(
      `components/setup-wizard.html?theme=${isDarkTheme ? "dark" : "light"}`
    );
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    try {
      ...
      document.querySelectorAll(".save-key").forEach((button) => {
        button.addEventListener("click", async (e) => {
          const provider = e.target
            .closest(".api-provider")
            .querySelector(".key-input").dataset.provider;
      ...
      showToast(
        "Failed to initialize. Please check if Ollama is running.",
        "error"
      );
    }
  });
  ...
  async function handleApiKeySave(provider) {
    ...
    try {
      ...
      if (provider === "openrouter") {
        ...
        await ApiKeyManager.saveEnabledModels(
          provider,
          models.map((m) => m.id)
        );
      }
  ...
  async function validateApiKey(provider, apiKey) {
    switch (provider) {
      ...
      case "gemini":
        ...
        const hasGeminiModel = geminiModels.some(
          (model) =>
      ...
      case "grok":
        try {
          ...
          await ApiKeyManager.saveEnabledModels("grok", ["grok-2-1212"]);
          return true;
        } catch (error) {
      ...
      case "perplexity":
        try {
          ...
          if (!apiKey.startsWith("pplx-")) {
            throw new Error(
              'Invalid Perplexity API key format. Key should start with "pplx-"'
            );
          }
  ...
  async function updateProviderStatus(provider) {
    ...
    const providerElement = document.querySelector(
      `.api-provider[data-provider="${provider}"]`
    );
    if (providerElement) {
    ...
    if (currentImage) {
      ...
      try {
        if (provider === "local") {
          ...
          capabilities = await ExternalModelService.getModelCapabilities(
            provider,
            modelId
          );
        }
    ...
    try {
          ...
          if (data.models.length > 0) {
            ...
            localSection.innerHTML = `
                            <div class="provider-header">
                                <img class="provider-logo" src="${chrome.runtime.getURL(
                                  "assets/ollama.jpg"
                                )}" alt="Ollama">
            ...
            if (filteredModels.length > 0) {
              ...
              filteredModels.forEach((model) => {
                ...
                if (`${provider}:${model.id}` === currentModel) {
                  option.classList.add("selected");
                  selectedText.textContent = model.name;
                  selectedLogo.src = chrome.runtime.getURL(
                    `assets/${logoFile}`
                  );
                  selectedLogo.style.display = "block";
                  hasSelectedModel = true;
      ...
      if (!hasAnyModels) {
        ...
        if (firstOption && firstOption.dataset.value) {
          ...
          selectedText.textContent =
            firstOption.querySelector("span").textContent;
          selectedLogo.src = chrome.runtime.getURL(
            `assets/${logoMap[provider]}`
          );
          selectedLogo.style.display = "block";
      ...
      newSelectItems.addEventListener("click", async (e) => {
        ...
        if (option && option.dataset.value) {
          // Only handle clicks on valid model options
  ...
  async function getProviderModels(
    ...
    const models = {
      ...
      grok: [
        ...
        // Vision model hidden from UI but still enabled in backend
      ],
    };
    ...
    for (const item of items) {
      if (item.type.indexOf("image") === 0) {
        ...
        reader.onload = async function (event) {
          ...
          // Switch to vision model internally if using Grok
          if (currentModel.startsWith("grok:")) {
            console.log("🔄 Switching to vision model due to image attachment");
            currentModel = "grok:grok-2-vision-1212";
          }
  ...
  function autoResizeTextarea() {
    ...
    userInput.style.overflowY =
      userInput.scrollHeight > 200 ? "auto" : "hidden";
  }
  ...
  function handleModelResponse(message) {
    ...
    if (!message.success) {
      ...
      showError(
        message.error || "An error occurred while generating the response"
      );
      return;
    }
    ...
    try {
      ...
      let contentContainer =
        currentAssistantMessage.querySelector(".content-container");
      if (!contentContainer) {
  ...
  userInput.addEventListener("input", () => {
    ...
    explainButtonContainer.style.display =
      text || currentImage ? "block" : "none";
    ...
    if (currentImage) {
      ...
      inputExplainButton.style.display =
        isVisionModel && text ? "block" : "none";
    } else {
...
Let's break this down:`;
  ...
  settingsButton.addEventListener("click", (e) => {
    ...
    document
      .querySelectorAll(".settings-menu, .api-key-dropdown")
      .forEach((el) => {
  ...
  async function showModelEditor() {
    ...
    const providerModels = await Promise.all(
      providers.map(async (provider) => {
        ...
        const enabledModels =
          (await ApiKeyManager.getEnabledModels(provider)) || [];
    ...
    modelEditor.innerHTML = `
            ...
            <div class="model-editor-content">
                ...
                <div class="model-editor-body">
                    ...
                    ${activeProviders
                      .map(({ provider, enabledModels, availableModels }) => {
                        if (provider === "openrouter") {
                          return `
    ...
    if (openRouterSection) {
      ...
      const addModelButton =
        openRouterSection.querySelector(".add-model-button");
      const validationMessage = openRouterSection.querySelector(
      ...
      addModelButton.addEventListener("click", async () => {
        ...
        const enabledModels =
          (await ApiKeyManager.getEnabledModels("openrouter")) || [];
      ...
      openRouterSection
        .querySelectorAll(".remove-model-button")
        .forEach((button) => {
    ...
    saveAllButton.addEventListener("click", async () => {
      ...
      try {
        for (const provider of providers) {
          const checkboxes = modelEditor.querySelectorAll(
    ...
    const defaultModelSelect = modelEditor.querySelector(
    ...
    defaultModelSelect.addEventListener("change", async () => {
      ...
      if (selectedModel) {
        ...
        if (!enabledModels.includes(modelId)) {
          ...
          const checkbox = modelEditor.querySelector(
            `input[data-provider="${provider}"][data-model="${modelId}"]`
          );
          if (checkbox) {
            ...
            const countSpan = modelList
              .closest(".provider-models")
              .querySelector(".model-count");
            const total = modelList.querySelectorAll(
              'input[type="checkbox"]'
            ).length;
            const checked = modelList.querySelectorAll(
              'input[type="checkbox"]:checked'
            ).length;
            countSpan.textContent = `${checked} of ${total} enabled`;
          }
    ...
    for (const item of items) {
      if (item.type.indexOf("image") === 0) {
        ...
        reader.onload = async function (event) {
          ...
          // Switch to vision model internally if using Grok
          if (currentModel.startsWith("grok:")) {
            console.log("🔄 Switching to vision model due to image drop");
            currentModel = "grok:grok-2-vision-1212";
          }
