...
export class ApiKeyManager {
  static async saveApiKey(provider, key) {
    try {
      ...
      if (provider === "grok") {
        ...
        await this.saveEnabledModels("grok", [
          "grok-2-1212",
          "grok-2-vision-1212",
        ]);
  ...
  static async getEnabledModels(provider) {
    console.log("📋 Getting enabled models for provider:", provider);
    if (await this.hasApiKey(provider)) {
      const result = await chrome.storage.local.get(
        `enabledModels_${provider}`
      );
      const models = result[`enabledModels_${provider}`] || [];
      console.log("✅ Retrieved enabled models:", {
        provider,
        models,
        count: models.length,
      });
